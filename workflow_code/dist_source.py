...
def distance_source(egsinp_path, egsphant_path, shape, tensor=False):
    ...
    if tensor:
        return torch.tensor(calculate_distance(origin_position=origin_position, shape=shape), dtype=torch.float16)
    else:
