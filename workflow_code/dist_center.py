...
def distance_center(egsinp_path, egsphant_path, shape):
    ...
    return calculate_distance(iso_center=iso_center, origin_position=origin_position, shape=shape)
    ...
    for x in tqdm(range(shape[0])):
        for y in range(shape[1]):
            for z in range(shape[2]):
                distance_vol[x, y, z] = distance(x, y, z, iso_center, origin_position)
...
def distance(x, y, z, iso_center, origin_position):
    ...
    vox = np.array([x, y, z])
    dist = np.linalg.norm(np.cross((vox - iso_center),iso_center - origin_position)) / np.linalg.norm(iso_center - origin_position)
