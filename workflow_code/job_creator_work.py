...
def create_listfile(path):
    ...
    used_files = [file_ for file_ in natsorted(os.listdir(path)) if not file_.startswith('._')
                  and not file_.startswith(".") and not file_ == f"{patient}_listfile.txt"]
...
def create_entire_job(n, gantry, par_jobs, ppn, nodes, beam_config, patient, iso_center=None):
    ...
    if iso_center is not None:
        ...
        target_filename = dcm_folder[:-1] + "_" + \
...
def setup_plan_calculation(patient, plan_file):
    ...
    for config in range(len(angles)):
        config_files.append(create_beam_config(
            patient, config, jaws[config], leafes[config]))
