...
def binary_mask(egsinp, egsphant, beam_config, px_sp=np.array([1.171875, 1.171875, 3]), SID=1435):

    angle, output_dim, shift, rotated_dim, rotated_iso_loc = setup_binary(egsinp, egsphant)

    percentual_increase, first_fac = scaling_factor(px_sp, SID, rotated_dim, rotated_iso_loc)
    ...
    entry_plane = get_entry_plane(leafes, jaws, first_fac,
                                  fieldsize_px[0], fieldsize_px[1],
                                  rotated_dim)
...
def setup_binary(egsinp, egsphant):
    ...
    rotated_dim = np.array([800, 800, num_slices + 2 * shift[2]], dtype=int)
...
def rotate_crop(angle, output_dim, shift, beam):
    ...
    center_window = np.array([np.round((rotated.shape[0] - output_dim[0])/2),
                              np.round((rotated.shape[1] - output_dim[1])/2),
                              np.round((rotated.shape[2] - output_dim[2])/2)], dtype=int)
...
def scaling_factor(px_sp, SID, rotated_dim, rotated_iso_loc):
    ...
    fac = [(px_sp[0] * (SID + i * px_sp[0])) / (2 * SID) / (px_sp[0] / 2)
...
def get_leaf_positions(config_path):
    ...
    if config_path.endswith(".dcm"):
        ...
        with dcmread(config_path) as dcm:
            ...
            leafes = np.array(
                dcm.BeamSequence[0].ControlPointSequence[0].
                BeamLimitingDevicePositionSequence[1][0x300a,
                                                      0x011c][:], float) / 10
            jaws = np.array(
                dcm.BeamSequence[0].ControlPointSequence[0].
                BeamLimitingDevicePositionSequence[0][0x300a,
                                                      0x011c][:], float)
...
def get_entry_plane(leafes, jaws, entry_plane_fac, transversal_size,
                    sagital_size, dim):
...
def scale_beam(fl, dim, scale):
    ...
    for slice in tqdm(range(dim[0])):
        vol[:, :, slice] = paddedzoom2(fl, scale[slice])

    return vol

#https://gist.github.com/i-namekawa/74a817683b0e68cee521
def paddedzoom2(img, zoomfactor):
...
def clipped_zoom(img, scale):
    ...
    out = resize(img, (new_w, new_h), interpolation=cv2.INTER_CUBIC)
...
#     binary = binary_mask(path, phant, beam_config)
