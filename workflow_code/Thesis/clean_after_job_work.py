...
def parse():
    ...
    parser.add_argument('filename', type=str, metavar='', help='filename')
    parser.add_argument('str_nhist', type=str, metavar='', help='number of histories')
    parser.add_argument('--dir', type=str, default="/work/ws/nemo/tu_zxoys08-EGS-0/egs_home/dosxyznrc/")
...
def clean_folder(filename, str_nhist, dir="/work/ws/nemo/tu_zxoys08-EGS-0/egs_home/dosxyznrc/"):
    ...
    assert os.path.isfile(
        dir + egsinp_file), f"{egsinp_file} .egsinp file got somehow deleted"
    assert os.path.isfile(
        dir + dose_file), f"{dose_file} .3ddose file got somehow deleted"
    assert os.path.isfile(
        beam_folder + beam_file),  f"{beam_file} beamfile .egsinp file got somehow deleted"
    assert os.path.isfile(
        beam_folder + beam_txt_file), f"{beam_txt_file} beamfile .txt file got somehow deleted"
