...
class trainingData():
            ...
            for i in range(self.num_leafes):
                if j == 0:
                    if self.MLC_iso[j, i] <= 0:
                        MLC_egsinp[j, i] = (((cil + math.sqrt(pow(radius, 2)-pow(math.cos(abs(self.MLC_iso[j, i]*10)*0.1/ssd)*radius, 2)))
                                            * abs(self.MLC_iso[j, i]*10)*0.1/ssd + math.cos(abs(self.MLC_iso[j, i]*10)*0.1/ssd)*radius)*(-1))
                    ...
                    if self.MLC_iso[j, i]*10 >= 0:
                        MLC_egsinp[j, i] = ((cil + math.sqrt(pow(radius, 2)-pow(math.cos(abs(self.MLC_iso[j, i]*10)*0.1/ssd)*radius, 2)))
                                            * abs(self.MLC_iso[j, i]*10)*0.1/ssd + math.cos(abs(self.MLC_iso[j, i]*10)*0.1/ssd)*radius)
                    if self.MLC_iso[j, i]*10 < 0:
                        MLC_egsinp[j, i] = (-(cil - math.sqrt(pow(radius, 2)-pow(math.cos(abs(self.MLC_iso[j, i]*10)*0.1/ssd)*radius, 2)))
                                            * abs(self.MLC_iso[j, i]*10)*0.1/ssd + math.cos(abs(self.MLC_iso[j, i]*10)*0.1/ssd)*radius)
    ...
    def calculate_new_jaw(self, cil=44.35-0.09, radius=13.0, ssd=143.5):
        ...
        if self.JAW_iso[0] <= 0:
            new_JAWS[0] = ((cil + math.sqrt(pow(radius, 2)-pow(math.cos(abs(self.JAW_iso[0]*10)*0.1/ssd)*radius, 2)))
                           * abs(self.JAW_iso[0]*10)*0.1/ssd + math.cos(abs(self.JAW_iso[0]*10)*0.1/ssd)*radius)*(-1)
        if self.JAW_iso[0] > 0:
            new_JAWS[0] = (-(cil - math.sqrt(pow(radius, 2)-pow(math.cos(abs(self.JAW_iso[0]*10)*0.1/ssd)*radius, 2)))
                           * abs(self.JAW_iso[0]*10)*0.1/ssd + math.cos(abs(self.JAW_iso[0]*10)*0.1/ssd)*radius)*(-1)
        if self.JAW_iso[1] >= 0:
        ...
        if self.JAW_iso[1] < 0:
            new_JAWS[1] = - (cil - math.sqrt(pow(radius, 2)-pow(math.cos(abs(self.JAW_iso[1]*10)*0.1/ssd)*radius, 2))
                             )*abs(self.JAW_iso[1]*10)*0.1/ssd + math.cos(abs(self.JAW_iso[1]*10)*0.1/ssd)*radius
...
field = trainingData(fieldsize=(10, 10), translation=(0, 0))
