...
class DoubleConv(nn.Module):
    def __init__(self, in_channels, out_channels, UQ=False, sample=False, p=0.1):
        ...
        self.sample = sample
        self.conv = block(in_channels, in_channels, 3, 1, 1, False)

        if UQ:
            self.conv.append(nn.Dropout(p=p))

        self.conv.extend(block(in_channels, out_channels, 3, 1, 1, False))
...
def test():
    ...
    model = Dose3DUNET(in_channels=5, out_channels=1, features=[64, 128, 256, 512])
