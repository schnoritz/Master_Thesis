...
def predict_plan(model, device, plan_path, patient, segments_path, shift=16):
    ...
    patient_segments = [x for x in os.listdir(segments_path) if patient + "_" in x and not "plan" in x and not "strctr" in x]
    ...
    for _, row in segment_weights.iterrows():
        ...
        final_prediction_dose = np.add(final_prediction_dose, predict_volume(masks, model, device, shift=shift) * row['weight'])
...
if __name__ == "__main__":
    ...
    for patient, entity in zip(patients, entities):
        ...
        model_path = "/mnt/qb/baumgartner/sgutwein84/save/prostate_trained/UNET_2234.pt"
        segment_dir = f"/mnt/qb/baumgartner/sgutwein84/training/training_{entity}/"
        save_path = f"/home/baumgartner/sgutwein84/container/plan_predictions_test/{patient}"
        ...
        torch.save(predicted_plan, f"{save_path}/{patient}_prediction.pt")
        torch.save(target_plan, f"{save_path}/{patient}_target.pt")
